...
export const ChatHistory = ({ classroomId }: { classroomId: string }) => {
  const { data: conversations, isLoading } =
    trpc.conversation.getPreviousConversations.useQuery({ classroomId });
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Your recent <span className="font-semibold">30</span> conversations
            are used for providing better context to our AI
          </p>
        </div>
        <Separator />
...
const ConversationCard = ({ conversation }: { conversation: Conversation }) => {
  ...
  return (
    <Card className="bg-neutral-100 shadow-none border border-border relative group">
      ...
      <CardContent className="py-3 text-gray-800 text-[15px]">
      ...
      <div className="opacity-0 group-hover:opacity-100 transition absolute bottom-2.5 right-3">
        <CustomTooltip text="Click to copy">
          ...
          >
            {copied ? (
              <Check className="w-3 h-3 cursor-pointer" />
            ) : (
